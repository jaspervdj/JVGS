...
namespace jvgs
{
    namespace sketch
    {
        ...
        void TranslateTransformCommand::apply(AffineTransformationMatrix &matrix,
                                              const vector<float> &arguments) const
        {
            if(arguments.size() == 1) {
                ...
                LogManager::getInstance()->error("Translate transformation needs 1 or 2 arguments, %d given.",
                        arguments.size());
            }
        }
    }
